...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - 相亲相爱一家人播放器';
    ...
    setupQualityEnhancementToggle();
    setTimeout(() => {
        findBetterQualityResource();
    }, 1800);
...
// 初始化播放器
function formatHlsQualityLabel(level, fallbackIndex) {
    if (!level) {
        return `线路 ${fallbackIndex + 1}`;
    }
    ...
    if (level.height) {
        ...
        parts.push(`${level.width}W`);
    } else if (level.name) {
        parts.push(level.name);
    } else {
        parts.push(`线路 ${fallbackIndex + 1}`);
    }

    const bitrate = level.bitrate || level.averageBitrate;
    if (bitrate) {
...
function getHlsQualityTooltip(hls) {
    if (!hls || !hls.levels || hls.levels.length === 0) {
        return '自动';
    }

    if (hls.currentLevel === -1) {
        const activeLevel = hls.levels[hls.loadLevel] || hls.levels[hls.nextLoadLevel];
        return activeLevel ? `自动 (${formatHlsQualityLabel(activeLevel, hls.loadLevel)})` : '自动';
    }

    return formatHlsQualityLabel(hls.levels[hls.currentLevel], hls.currentLevel);
}

function getHighestHlsLevelIndex(hls) {
    ...
    return hls.levels.reduce((bestIndex, level, index) => {
        ...
        const bestBitrate = best.bitrate || best.averageBitrate || 0;
        const levelBitrate = level.bitrate || level.averageBitrate || 0;
...
function updateHlsQualitySetting(hls) {
    ...
    const levels = hls.levels
        ...
        .sort((a, b) => {
            const aHeight = a.level.height || 0;
            const bHeight = b.level.height || 0;
            const aBitrate = a.level.bitrate || a.level.averageBitrate || 0;
            const bBitrate = b.level.bitrate || b.level.averageBitrate || 0;
            return (bHeight - aHeight) || (bBitrate - aBitrate) || (a.index - b.index);
        });

    const selector = [
        {
            value: -1,
            name: 'hls-quality-auto',
            default: hls.currentLevel === -1,
            html: '自动'
        },
        ...levels.map(({ level, index }) => ({
            value: index,
            name: `hls-quality-${index}`,
            default: hls.currentLevel === index,
            html: formatHlsQualityLabel(level, index)
        }))
    ];

    art.setting.update({
        width: Artplayer.SETTING_ITEM_WIDTH,
        name: 'hls-quality',
        html: '清晰度',
        tooltip: getHlsQualityTooltip(hls),
        selector,
        onSelect(item) {
            hls.currentLevel = item.value;
            updateHlsQualitySetting(hls);
            if (art && art.notice) {
                art.notice.show = item.value === -1 ? '清晰度：自动' : `清晰度：${item.html}`;
            }
            return item.value === -1 ? '自动' : item.html;
        }
    });
}
...
function applyQualityEnhancement() {
    ...
    const toggle = document.getElementById('qualityEnhanceToggle');
    const presetSelect = document.getElementById('qualityEnhancePreset');
    const enabled = localStorage.getItem('qualityEnhanceEnabled') === 'true';
    const preset = localStorage.getItem('qualityEnhancePreset') || 'natural';

    if (container) {
        container.classList.toggle('quality-enhanced', enabled);
        ['natural', 'sharp', 'vivid', 'dark', 'soft', 'glfx'].forEach((name) => {
            container.classList.toggle(`quality-preset-${name}`, enabled && preset === name);
        });
    }
    ...
    updateGlfxEnhancement(enabled && preset === 'glfx');
}

function setupQualityEnhancementToggle() {
    const toggle = document.getElementById('qualityEnhanceToggle');
    const presetSelect = document.getElementById('qualityEnhancePreset');
    ...
    toggle.addEventListener('change', function () {
        localStorage.setItem('qualityEnhanceEnabled', this.checked ? 'true' : 'false');
        applyQualityEnhancement();
        if (art && art.notice) {
            art.notice.show = this.checked ? '画质增强：开' : '画质增强：关';
        }
    });
...
const GLFX_SCRIPT_URLS = [
    'https://evanw.github.io/glfx.js/glfx.js',
    'https://cdn.jsdelivr.net/gh/evanw/glfx.js/glfx.js'
];
let glfxLoadPromise = null;
let glfxState = {
    active: false,
    canvas: null,
    texture: null,
...
function loadGlfxLibrary() {
    if (window.fx && typeof window.fx.canvas === 'function') {
        return Promise.resolve(window.fx);
    }

    if (glfxLoadPromise) {
        return glfxLoadPromise;
    }

    glfxLoadPromise = new Promise((resolve, reject) => {
        let index = 0;

        function tryLoad() {
            if (index >= GLFX_SCRIPT_URLS.length) {
                reject(new Error('glfx.js 加载失败'));
                return;
            }

            const script = document.createElement('script');
            script.src = GLFX_SCRIPT_URLS[index++];
            script.async = true;
            script.onload = () => {
                if (window.fx && typeof window.fx.canvas === 'function') {
                    resolve(window.fx);
...
function stopGlfxEnhancement() {
    glfxState.active = false;
    if (glfxState.rafId) {
        cancelAnimationFrame(glfxState.rafId);
    }
    if (glfxState.canvas && glfxState.canvas.parentNode) {
        glfxState.canvas.parentNode.removeChild(glfxState.canvas);
    }
    glfxState = {
        active: false,
        canvas: null,
...
function syncGlfxCanvasSize(canvas, video, wrapper) {
    const width = wrapper.clientWidth || video.clientWidth || video.videoWidth || 1;
    const height = wrapper.clientHeight || video.clientHeight || video.videoHeight || 1;
...
async function updateGlfxEnhancement(shouldEnable) {
    if (!shouldEnable) {
        stopGlfxEnhancement();
        return;
    }
    ...
    const wrapper = art && art.template && art.template.$player;
    if (!video || !wrapper) {
        return;
    }

    try {
        const fxLib = await loadGlfxLibrary();
        if (localStorage.getItem('qualityEnhanceEnabled') !== 'true' || localStorage.getItem('qualityEnhancePreset') !== 'glfx') {
            return;
        }

        const canvas = fxLib.canvas();
        canvas.className = 'glfx-video-canvas';
        ...
        const render = () => {
            if (!glfxState.active || !glfxState.canvas || !glfxState.texture || !video || video.readyState < 2) {
                glfxState.rafId = requestAnimationFrame(render);
                return;
            }

            try {
                syncGlfxCanvasSize(canvas, video, wrapper);
                texture.loadContentsOf(video);
                canvas
                    .draw(texture)
                    .brightnessContrast(0.02, 0.12)
                    .hueSaturation(0, 0.08)
                    .unsharpMask(18, 0.45)
                    .update();
            } catch (error) {
                stopGlfxEnhancement();
...
function buildResourceDetailApiParams(sourceKey) {
...
const QUALITY_PROBE_CACHE_KEY = 'qualityProbeCacheV1';
const QUALITY_PROBE_CACHE_TTL = 7 * 24 * 60 * 60 * 1000;

function getQualityCacheKey(sourceKey, vodId, episodeIndex = currentEpisodeIndex) {
    return `${sourceKey}:${vodId}:${episodeIndex}`;
}

function getQualityProbeCache() {
    try {
        return JSON.parse(localStorage.getItem(QUALITY_PROBE_CACHE_KEY) || '{}');
    } catch (error) {
        return {};
    }
}

function readCachedQualityInfo(sourceKey, vodId, episodeIndex = currentEpisodeIndex) {
    const cache = getQualityProbeCache();
    const entry = cache[getQualityCacheKey(sourceKey, vodId, episodeIndex)];
    if (!entry || Date.now() - entry.timestamp > QUALITY_PROBE_CACHE_TTL) {
        return null;
    }
...
function writeCachedQualityInfo(sourceKey, vodId, value, episodeIndex = currentEpisodeIndex) {
    try {
        const cache = getQualityProbeCache();
        cache[getQualityCacheKey(sourceKey, vodId, episodeIndex)] = {
            timestamp: Date.now(),
            value
        };
...
function getQualityGuessFromUrl(url) {
    const text = String(url || '').toLowerCase();
    if (/2160|4k|uhd/.test(text)) {
        return { label: '疑似4K', className: 'quality-badge-hd', score: 4000 };
    }
    if (/1080|fhd|fullhd|blue|bluray|bd/.test(text)) {
        return { label: '疑似1080P', className: 'quality-badge-hd', score: 1080 };
    }
    if (/720|hd/.test(text)) {
        return { label: '疑似720P', className: 'quality-badge-mid', score: 720 };
    }
    if (/480|sd/.test(text)) {
        return { label: '疑似480P', className: 'quality-badge-single', score: 480 };
    }
    return { label: '清晰度未知', className: 'quality-badge-unknown', score: 0 };
}

function getQualityFromPlaylist(content) {
    if (!content || typeof content !== 'string') {
        return { label: '清晰度未知', className: 'quality-badge-unknown', score: 0 };
    }

    const resolutions = [...content.matchAll(/RESOLUTION=(\d+)x(\d+)/gi)]
        .map(match => parseInt(match[2], 10))
        .filter(Boolean);

    if (resolutions.length > 0) {
        const maxHeight = Math.max(...resolutions);
        const hasMultiple = new Set(resolutions).size > 1;
        return {
            label: hasMultiple ? `多清晰度 ${maxHeight}P` : `${maxHeight}P`,
            className: maxHeight >= 1080 ? 'quality-badge-hd' : (maxHeight >= 720 ? 'quality-badge-mid' : 'quality-badge-single'),
            score: hasMultiple ? maxHeight + 10000 : maxHeight
        };
    }

    const bandwidths = [...content.matchAll(/BANDWIDTH=(\d+)/gi)]
    ...
    if (bandwidths.length > 1) {
        ...
        return {
            label: `多码率 ${(maxBandwidth / 1000000).toFixed(1)}Mbps`,
            className: maxBandwidth >= 3500000 ? 'quality-badge-hd' : 'quality-badge-mid',
            score: maxBandwidth >= 3500000 ? 9000 : 7000
        };
    }

    if (content.trim().startsWith('#EXTM3U')) {
        return { label: '单清晰度', className: 'quality-badge-single', score: 100 };
    }

    return { label: '清晰度未知', className: 'quality-badge-unknown', score: 0 };
}

async function probeEpisodeQuality(episodeUrl) {
    const urlGuess = getQualityGuessFromUrl(episodeUrl);
    if (!episodeUrl || !/\.m3u8($|\?)/i.test(episodeUrl)) {
        return urlGuess.score > 0 ? urlGuess : { label: '非HLS源', className: 'quality-badge-unknown', score: 0 };
    }
    ...
    try {
        ...
        const playlistQuality = getQualityFromPlaylist(await response.text());
        return playlistQuality.score > 0 ? playlistQuality : urlGuess;
    } catch (error) {
        clearTimeout(timeoutId);
        return urlGuess;
    }
...
async function getResourceQualityInfo(sourceKey, vodId) {
    const cached = readCachedQualityInfo(sourceKey, vodId);
    if (cached) {
        return cached;
    }

    const apiParams = buildResourceDetailApiParams(sourceKey);
    if (!apiParams) {
        return { label: '配置无效', className: 'quality-badge-unknown', score: 0 };
    }
    ...
    try {
        ...
        if (episodes.length === 0) {
            return { label: '无播放源', className: 'quality-badge-unknown', score: 0 };
        }

        const targetEpisode = episodes[Math.min(currentEpisodeIndex, episodes.length - 1)] || episodes[0];
        const qualityInfo = await probeEpisodeQuality(targetEpisode);
        writeCachedQualityInfo(sourceKey, vodId, qualityInfo);
        return qualityInfo;
    } catch (error) {
        return { label: '清晰度未知', className: 'quality-badge-unknown', score: 0 };
    }
...
function showBetterQualityPrompt(candidate) {
    if (!candidate || document.getElementById('betterQualityPrompt')) {
        return;
    }

    const prompt = document.createElement('div');
    prompt.id = 'betterQualityPrompt';
    prompt.className = 'fixed right-4 bottom-4 z-[10002] max-w-xs rounded-lg border border-[#333] bg-[#111] p-3 shadow-lg';
    prompt.innerHTML = `
        <div class="text-sm font-semibold text-white">发现更高清源</div>
        <div class="mt-1 text-xs text-gray-300">${candidate.sourceName} · ${candidate.quality.label}</div>
        <div class="mt-3 flex justify-end gap-2">
            <button type="button" class="px-2 py-1 text-xs rounded border border-[#333] text-gray-300 hover:bg-[#222]" data-action="dismiss">稍后</button>
            <button type="button" class="px-2 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700" data-action="switch">切换</button>
        </div>
    `;
    ...
    prompt.addEventListener('click', function (event) {
        ...
        if (action === 'switch') {
            prompt.remove();
            switchToResource(candidate.sourceKey, candidate.vodId);
        }
    });
...
async function findBetterQualityResource() {
    ...
    if (!currentVideoTitle || selectedAPIs.length === 0) {
        return;
    }

    let currentQuality = { score: 0 };
    if (currentSourceCode && currentVideoId) {
        currentQuality = await getResourceQualityInfo(currentSourceCode, currentVideoId);
    } else if (currentVideoUrl) {
        currentQuality = await probeEpisodeQuality(currentVideoUrl);
    }

    const resourceOptions = selectedAPIs
        .filter(sourceKey => String(sourceKey) !== String(currentSourceCode))
        .map((sourceKey) => {
            if (API_SITES[sourceKey]) {
                return { key: sourceKey, name: API_SITES[sourceKey].name };
            }
            const customIndex = parseInt(sourceKey.replace('custom_', ''), 10);
            if (customAPIs[customIndex]) {
                return { key: sourceKey, name: customAPIs[customIndex].name || '自定义资源' };
            }
...
async function showSwitchResourceModal() {
    ...
    // 对结果进行排序
    let sortedResults = Object.entries(allResults).sort(([keyA, resultA], [keyB, resultB]) => {
        // 当前播放的源放在最前面
    ...
    await Promise.all(sortedResults.map(async ([sourceKey, result]) => {
        qualityInfoMap[sourceKey] = await getResourceQualityInfo(sourceKey, result.vod_id);
    }));

    sortedResults = sortedResults.sort(([keyA, resultA], [keyB, resultB]) => {
        const isCurrentA = String(keyA) === String(currentSourceCode) && String(resultA.vod_id) === String(currentVideoId);
        const isCurrentB = String(keyB) === String(currentSourceCode) && String(resultB.vod_id) === String(currentVideoId);
        ...
        const scoreA = qualityInfoMap[keyA]?.score || 0;
        const scoreB = qualityInfoMap[keyB]?.score || 0;
        if (scoreA !== scoreB) return scoreB - scoreA;

        return selectedAPIs.indexOf(keyA) - selectedAPIs.indexOf(keyB);
    });
    ...
    for (const [sourceKey, result] of sortedResults) {
        ...
        const qualityInfo = qualityInfoMap[sourceKey] || { label: '清晰度未知', className: 'quality-badge-unknown' };
        ...
        html += `
            <div class="relative group ${isCurrentSource ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 transition-transform'}" 
                ...
                <div class="mt-1">
                    ...
                    <span class="quality-badge ${qualityInfo.className}">${qualityInfo.label}</span>
                </div>
                ${isCurrentSource ? `
